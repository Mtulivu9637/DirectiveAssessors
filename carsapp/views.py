...
def admindashboard(request):
    if request.user.is_superuser:  # Check if the user is an admin
        files = AssessmentReports.objects.all()  # Fetch all uploaded files
        return render(request, 'adminhome.html', {'files': files})
    else:
        return redirect('login_assessor')  # Redirect if not admin
...
def add_assessor(request):
    if request.method == 'POST':
        ...
        # Add assessor logic
        AssessorProfile.objects.create(names=name, id_number=id_number, username=username, password=password)
...
def login_assessor(request):
    if request.method == "POST":
        ...
        # First, authenticate the user using Django's built-in system
        ...
        if user:
            ...
            if user.is_superuser:
                ...
                login(request, user)
                return redirect('admindashboard')  # Admin dashboard redirect
            else:
                # Deny access for non-admin users trying to log in as admin
                return render(request, 'assessor_login.html', {'error': 'Access denied'})
        else:
            # If authentication fails, check if the user is an assessor
            try:
                assessor = AssessorProfile.objects.get(username=username)

                if check_password(password, assessor.password):
                    # Assessor credentials are correct
                    login(request, assessor)  # Log in the assessor
                    return redirect('dashboard')  # Regular assessor dashboard redirect
                else:
                    # Password is incorrect
                    return render(request, 'assessor_login.html', {'error': 'Invalid credentials'})
            except AssessorProfile.DoesNotExist:
                # User not found in AssessorProfile
...
def logout_view(request):
    ...
    return redirect('login_assessor')  # Redirect to login page after logout